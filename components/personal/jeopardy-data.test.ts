...
import { fetchPredictions, normalizePredictions, summarizePredictions, formatPredictionDate } from "./jeopardy-data";
...
test("rejects invalid probabilities, dates, outcomes, and duplicate episodes", () => {
  const valid: unknown[] = [1, "2026-07-21", "A", "ONE", 1, 70, 1, "Closed"];
  for (const [column, value] of [[5, 101], [5, -1], [4, 2], [6, "unknown"], [1, "2026-02-30"], [7, "unknown"]] as [number, unknown][]) {
    const row = [...valid];
    row[column] = value;
...
test("reads a downloaded XLSX and respects both Excel date systems", async () => {
...
test("rejects upstream errors and HTML responses instead of displaying bogus data", async () => {
  vi.spyOn(globalThis, "fetch").mockImplementation( async () => new Response("Unavailable", { status: 503 }));
  await assert.rejects(fetchPredictions(new AbortController().signal), /503/);
  vi.restoreAllMocks();
  vi.spyOn(globalThis, "fetch").mockImplementation( async () => new Response("<html>Sign in</html>"));
